...
CREATE TABLE PaymentMetthod (
    ID INT PRIMARY KEY IDENTITY(1,1),
    PaymentMetthodName VARCHAR(50) NOT NULL,
    Description VARCHAR(255) NOT NULL
);
GO
...
CREATE TABLE Seat (
    ...
    SeatClass VARCHAR(50) CHECK (SeatClass IN ('Primera Clase', 'Ejecutiva', 'Premium', 'Economica')),
...
CREATE TABLE Coupon (
    ...
    DateOfRedemption DATE CHECK (DateOfRedemption <= GETDATE()),  -- La fecha de redención debe ser en el pasado o hoy
    Class VARCHAR(50) CHECK (Class IN ('Primera Clase', 'Ejecutiva', 'Premium', 'Economica')),  -- Ajuste de clases
    Standby VARCHAR(50) CHECK (Standby IN ('Si', 'No')),  -- Valores permitidos: 'Si' o 'No'
    MealCode CHAR(1) NOT NULL CHECK (MealCode IN ('A', 'B', 'C', 'D')),  -- Ajuste del MealCode con 4 opciones posibles
    TicketingCode INT NOT NULL,
    FlightID INT,
    CONSTRAINT FK_Coupon_Ticket FOREIGN KEY (TicketingCode) REFERENCES Ticket(TicketingCode) ON DELETE CASCADE,
    CONSTRAINT FK_Coupon_Flight FOREIGN KEY (FlightID) REFERENCES Flight(FlightID) ON DELETE CASCADE
);
GO
...
-- tabla CrewMember------------------------------------
--TRUNCATE TABLE CrewMember;
WITH RandomData AS (
    SELECT 
        DISTINCT N.Name AS Name,                   -- Nombre único
        C.CountryName AS Nationality,              -- Nacionalidad aleatoria (puede repetirse)
...
FROM 
    RandomData
ORDER BY 
    NEWID();  -- Ordenar aleatoriamente para insertar
GO

----------FlightNumber------------------------------------------------------
--TRUNCATE TABLE FlightNumber;
...
------------------------------------FrequentFlyerCard------------------------------------
--TRUNCATE TABLE FrequentFlyerCard;
...
DECLARE @EndDate DATE = (SELECT MAX(FlightDate) FROM Flight); -- Fecha del último vuelo en la tabla Flight
...
------------------------FlightAssignment ------------------------------------------------
--TRUNCATE TABLE FlightAssignment;
...
-- Asignamos el estado inicial de todos los vuelos a "Programado"
WHILE @AssignmentCounter <= 3000
BEGIN
    SET @AssignmentFlightID = @AssignmentCounter;
    SET @FlightStatusID = 1; -- Estado "Programado"

    SET @AssignmnetDescription = 'El vuelo ' + CAST(@AssignmentFlightID AS VARCHAR(10)) + ' está programado.';
...
-- Segundo paso: algunos vuelos pasan de "Programado" a "Retrasado" o "Cancelado"
DECLARE @DelayedOrCanceledCounter INT = 1;

WHILE @DelayedOrCanceledCounter <= 3000
BEGIN
    SET @AssignmentFlightID = @DelayedOrCanceledCounter;

    -- Decidir si el vuelo pasa a "Retrasado" o "Cancelado"
    SET @FlightStatusID = 
        CASE 
            WHEN RAND() < 0.8 THEN 2 -- 80% de probabilidad de "Retrasado"
            ELSE 3                    -- 20% de probabilidad de "Cancelado"
        END;
    ...
    SET @DelayedOrCanceledCounter = @DelayedOrCanceledCounter + 1;
END;

-- Tercer paso: los vuelos que no fueron cancelados pasan a "En vuelo"
DECLARE @InFlightCounter INT = 1;

WHILE @InFlightCounter <= 3000
BEGIN
    SET @AssignmentFlightID = @InFlightCounter;

    -- Verificar si el vuelo no fue cancelado
    IF NOT EXISTS (
        SELECT 1 
        FROM FlightAssignment 
        WHERE FlightID = @AssignmentFlightID AND FlightStatusID = 3 -- Estado "Cancelado"
    )
    BEGIN
        SET @FlightStatusID = 4; -- Estado "En vuelo"
        SET @AssignmnetDescription = 'El vuelo ' + CAST(@AssignmentFlightID AS VARCHAR(10)) + ' está en vuelo.';

        INSERT INTO FlightAssignment (Description, FlightID, FlightStatusID)
        VALUES (@AssignmnetDescription, @AssignmentFlightID, @FlightStatusID);
    END

    SET @InFlightCounter = @InFlightCounter + 1;
END;

-- Cuarto paso: todos los vuelos que no fueron cancelados pasan a "Aterrizado"
DECLARE @LandedCounter INT = 1;

WHILE @LandedCounter <= 3000
BEGIN
    SET @AssignmentFlightID = @LandedCounter;
    ...
    BEGIN
        SET @FlightStatusID = 5; -- Estado "Aterrizado"
        SET @AssignmnetDescription = 'El vuelo ' + CAST(@AssignmentFlightID AS VARCHAR(10)) + ' ha aterrizado.';

        INSERT INTO FlightAssignment (Description, FlightID, FlightStatusID)
        VALUES (@AssignmnetDescription, @AssignmentFlightID, @FlightStatusID);
    END

    SET @LandedCounter = @LandedCounter + 1;
END;
...
-- Obtener el total de modelos de avión desde la tabla PlaneModel
DECLARE @TotalPlaneModels INT;
SELECT @TotalPlaneModels = COUNT(*) FROM PlaneModel;

WHILE @C <= @TotalPlaneModels
BEGIN
    -- PlaneModelID actual
    SET @PlaneModelID1 = @C;
    ...
    BEGIN
        ...
        IF @SeatCounter <= @MaxSeats * 0.15
            SET @SeatClass = 'Primera Clase';       -- 15% de los asientos son de Primera Clase
        ELSE IF @SeatCounter <= @MaxSeats * 0.35
            SET @SeatClass = 'Ejecutiva';           -- 20% adicional para Ejecutiva
        ELSE IF @SeatCounter <= @MaxSeats * 0.45
            SET @SeatClass = 'Premium';             -- 10% adicional para Premium
        ELSE
            SET @SeatClass = 'Economica';           -- 55% restante para Económica
        ...
        INSERT INTO Seat (SeatClass, Number, Location, PlaneModelID)
        VALUES (@SeatClass, @Number, @Location, @PlaneModelID1);
...
DECLARE @PreviousGoalAirportID INT;
DECLARE @InitialPlaneModelID INT;

-- Seleccionar aleatoriamente el 85% de los vuelos para asignar escalas
DECLARE VuelosConEscalas CURSOR FOR
SELECT TOP (@MaxVuelosConEscalas) FlightID, PlaneModelID -- Incluimos el modelo de avión inicial
FROM Flight
ORDER BY NEWID();

OPEN VuelosConEscalas;
FETCH NEXT FROM VuelosConEscalas INTO @ScaleFlightID, @InitialPlaneModelID;
...
BEGIN
    ...
    SET @PreviousGoalAirportID = NULL; -- Reset para cada vuelo
    SET @ScalePlaneModelID = @InitialPlaneModelID; -- Usar el modelo de avión inicial al comenzar
    ...
    BEGIN
        ...
        -- Definir aeropuertos de inicio y destino para una ruta continua
        IF @Escala = 1
            SET @ScaleStartAirportID = (SELECT TOP 1 AirportID FROM Airport ORDER BY NEWID());
        ELSE
            SET @ScaleStartAirportID = @PreviousGoalAirportID;

        -- Asegurar que GoalAirportID sea diferente de StartAirportID
        SET @ScaleGoalAirportID = (SELECT TOP 1 AirportID FROM Airport WHERE AirportID <> @ScaleStartAirportID ORDER BY NEWID());

        -- Lógica para cambio de modelo de avión
        IF @StopoverDuration > '05:00:00' -- Solo para escalas largas
        BEGIN
            -- Cambiar de modelo de avión con un 50% de probabilidad
            IF RAND() < 0.5
            BEGIN
                SET @ScalePlaneModelID = CASE 
                                            WHEN RAND() < 0.5 THEN FLOOR(RAND() * 5) + 1       -- 50% entre 1 y 5 (corta distancia)
                                            WHEN RAND() < 0.8 THEN FLOOR(RAND() * 5) + 6       -- 30% entre 6 y 10 (media distancia)
                                            ELSE FLOOR(RAND() * 5) + 11                        -- 20% entre 11 y 15 (larga distancia)
                                        END;
            END
        END
        ...
        -- Guardar el GoalAirportID actual para la siguiente escala en el mismo vuelo
        SET @PreviousGoalAirportID = @ScaleGoalAirportID;
...
WHILE @C3 <= 14000
BEGIN
    -- FlightID entre 1 y 3000 para abarcar todos los vuelos
    SET @FlightID3 = FLOOR(RAND() * 3000) + 1;

    -- RoleID entre 1 y 5 (Piloto, Copiloto, Auxiliar de Vuelo, Mecánico, Administrador)
    SET @RoleID = FLOOR(RAND() * 5) + 1;

    -- CrewMemberID entre 1 y 569
    SET @CrewMemberID = FLOOR(RAND() * 569) + 1;

    -- Asignación de ScaleID solo para roles específicos
    IF @RoleID IN (1, 2) -- Piloto y Copiloto asignados al vuelo completo
    BEGIN
        SET @ScaleID = NULL; -- Sin escala específica para vuelo completo
    END
    ELSE
    BEGIN
        -- ScaleID aleatorio entre 1 y el número total de escalas existentes, si el vuelo tiene escalas
        IF EXISTS (SELECT 1 FROM Scale WHERE FlightID = @FlightID3)
            SET @ScaleID = (SELECT TOP 1 ScaleID FROM Scale WHERE FlightID = @FlightID3 ORDER BY NEWID());
        ELSE
            SET @ScaleID = NULL; -- Sin escala si el vuelo no tiene escalas
    END

    -- Insertar en RoleAssignment
...
-----------------------StatusAssignment-------------
--TRUNCATE TABLE StatusAssignment;
DECLARE @C5 INT = 1;
...
-- Obtener el total de reservas en la tabla Reservation
SELECT @TotalReservas = COUNT(*) FROM [soporte_AERO].[dbo].[Reservation];

-- Asignar el estado inicial "Pendiente" para cada reserva usando el límite dinámico
WHILE @C5 <= @TotalReservas  
BEGIN
    INSERT INTO [soporte_AERO].[dbo].[StatusAssignment] (Descripcion, ReservationID, StatusID)
...
DECLARE @CouponTicketingCode INT;
DECLARE @CouponClass NVARCHAR(50);
DECLARE @CouponFlightID INT;
DECLARE @CouponFlightDate DATE;
DECLARE @CouponDateOfRedemption DATETIME;
DECLARE @CouponStandby NVARCHAR(50);
DECLARE @CouponMealCode CHAR(1);
DECLARE @CouponScaleCount INT;

-- Cursor para recorrer todos los tickets confirmados
...
OPEN TicketCursor;
FETCH NEXT FROM TicketCursor INTO @CouponTicketingCode, @CouponClass, @CouponFlightID, @CouponFlightDate;

WHILE @@FETCH_STATUS = 0
BEGIN
    -- Primer cupón para el vuelo principal (sin escala)
    SET @CouponDateOfRedemption = @CouponFlightDate;

    -- Generar valores de Standby y MealCode basados en la clase del ticket
    SET @CouponStandby = CASE WHEN RAND() < 0.1 THEN 'Si' ELSE 'No' END;
    SET @CouponMealCode = CASE 
                            WHEN @CouponClass = 'Primera Clase' THEN 'A'
                            WHEN @CouponClass = 'Ejecutiva' THEN 'B'
                            WHEN @CouponClass = 'Premium' THEN 'C'
                            ELSE 'D' -- Económica
                          END;

    -- Insertar el cupón del vuelo principal
    INSERT INTO Coupon (TicketingCode, DateOfRedemption, Class, Standby, MealCode, FlightID)
    VALUES (@CouponTicketingCode, @CouponDateOfRedemption, @CouponClass, @CouponStandby, @CouponMealCode, @CouponFlightID);

    -- Cursor para recorrer escalas del vuelo actual
    DECLARE ScaleCursor CURSOR FOR
    SELECT 
        ScaleID,
        DATEADD(HOUR, 5 + FLOOR(RAND() * 11), @CouponDateOfRedemption) AS ScaleRedemptionDate  -- Incremento de 5 a 15 horas desde el último cupón
    FROM 
        Scale
    WHERE 
        FlightID = @CouponFlightID
    ORDER BY 
        ScaleID;

    OPEN ScaleCursor;
    FETCH NEXT FROM ScaleCursor INTO @CouponScaleCount, @CouponDateOfRedemption;

    -- Insertar cupones para cada escala
    WHILE @@FETCH_STATUS = 0
    BEGIN
        -- Insertar el cupón para la escala
        INSERT INTO Coupon (TicketingCode, DateOfRedemption, Class, Standby, MealCode, FlightID)
        VALUES (@CouponTicketingCode, @CouponDateOfRedemption, @CouponClass, @CouponStandby, @CouponMealCode, @CouponFlightID);

        -- Obtener la siguiente escala y calcular la nueva fecha de redención
        FETCH NEXT FROM ScaleCursor INTO @CouponScaleCount, @CouponDateOfRedemption;
    END;
...
BEGIN
    ...
    BEGIN
        ...
        -- Generar una fecha de pago entre la fecha de reserva y la fecha de vuelo
        IF DATEDIFF(DAY, @PaymentReservationDate, @PaymentFlightDate) = 0
        BEGIN
            -- Si el pago es el mismo día que el vuelo, generamos una hora antes de BoardingTime
            SET @PaymentDate = DATEADD(HOUR, FLOOR(RAND() * DATEPART(HOUR, @PaymentBoardingTime)), @PaymentFlightDate);
            SET @PaymentDate = DATEADD(MINUTE, FLOOR(RAND() * DATEPART(MINUTE, @PaymentBoardingTime)), @PaymentDate);
        END
        ELSE
        BEGIN
            -- Generar @PaymentDate entre la reserva y el vuelo
        ...
        -- Insertar el registro de pago en la tabla Payment
...
-----------------------PiecesOfLuggage--------------------
--TRUNCATE TABLE PiecesOfLuggage;
...
------------------------AvailableSeat------------------------------------
--TRUNCATE TABLE AvailableSeat;
-- Crear tabla temporal para almacenar los asientos asignados y disponibles por vuelo
CREATE TABLE #TempAvailableSeat (
    TempFlightID INT,
    TempSeatID INT,
    TempCouponID INT
);

-- Paso 1: Insertar todos los asientos disponibles por vuelo y modelo de avión en la tabla temporal, con TempCouponID como NULL
INSERT INTO #TempAvailableSeat (TempFlightID, TempSeatID, TempCouponID)
SELECT f.FlightID, s.SeatID, NULL
FROM Flight f
JOIN Seat s ON f.PlaneModelID = s.PlaneModelID;

-- Paso 2: Asignar asientos a los cupones en función de la clase, sin usar cursores
...
WITH RankedCoupons AS (
    SELECT 
        ...
        c.FlightID,
        c.Class,
        ROW_NUMBER() OVER (PARTITION BY c.FlightID, c.Class ORDER BY c.CouponID) AS RowNum
    FROM Coupon c
),
RankedSeats AS (
    SELECT 
        ...
        t.TempFlightID,
        s.SeatClass,
        ROW_NUMBER() OVER (PARTITION BY t.TempFlightID, s.SeatClass ORDER BY t.TempSeatID) AS RowNum
    FROM #TempAvailableSeat t
    JOIN Seat s ON t.TempSeatID = s.SeatID
    WHERE t.TempCouponID IS NULL
)

-- Actualizar los asientos disponibles con los cupones correspondientes, usando la correlación de RowNum
UPDATE t
SET TempCouponID = rc.CouponID
FROM #TempAvailableSeat t
JOIN RankedSeats rs ON t.TempSeatID = rs.TempSeatID AND t.TempFlightID = rs.TempFlightID
JOIN RankedCoupons rc ON t.TempFlightID = rc.FlightID AND rs.SeatClass = rc.Class AND rs.RowNum = rc.RowNum;

-- Insertar los datos finales de la tabla temporal en AvailableSeat
INSERT INTO AvailableSeat (SeatID, FlightID, CouponID)
SELECT TempSeatID, TempFlightID, TempCouponID
FROM #TempAvailableSeat;

-- Limpiar la tabla temporal
DROP TABLE #TempAvailableSeat;
